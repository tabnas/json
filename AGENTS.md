...
1. Bump all **three** version sites together — `ts/package.json`, `VERSION`
   in `ts/src/json.ts` and `const VERSION` in `go/json.go`. They are held
...
6. Confirm `npm view @tabnas/json@$V version`, and **query both tags
   exactly**:
...
- `publish-go V=x.y.z` breaks the version invariant: it `sed`s and stages
  **only** `go/json.go`, leaving `ts/package.json` and `VERSION` in
  `ts/src/json.ts` on the previous version — the exact state the version
