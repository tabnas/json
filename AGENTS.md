...
| [`rs/`](rs/) | Rust port — the `tabnas-json` crate. Plugin in `src/lib.rs`. Depends on the `tabnas` crate via a `path` dependency (sibling checkout). Library only: no CLI. See [`rs/AGENTS.md`](rs/AGENTS.md). |
...
1. **The shared fixtures pass in ALL THREE runtimes.** `test/spec/*.tsv`
   ...
   (rule 3 above).
2. **The external conformance suite still passes in full.** TS and Go
   ...
   failure, not a skip. **Rust does not run that corpus**; its second
   opinion is `serde_json`, applied to every valid shared-fixture row in
...
1. Bump all **six** version sites together — `ts/package.json`, `VERSION`
   ...
   not hand-edited. Drift is caught by `ts/test/version.test.js`,
...
same directory. **The Rust port does not run this corpus**; everywhere
below, "both runtimes" means TS and Go. **If the corpus is absent they
FAIL, they do not skip** — a conformance suite that quietly does not run
reports a green tick that is a lie. Both also assert the corpus census
(95/188/35, 318 total) before grading, so narrowing it goes red instead
of inflating the pass rate.
