...
| [`test/spec/`](test/spec/) | Shared `.tsv` conformance fixtures (`valid.tsv` = `input → expected`, `errors.tsv` / `reject-extended.tsv` = `input → ERROR:<code>`). Auto-discovered and run by both suites. `rfc8259-gaps.tsv` is **deliberately red**. See [`test/AGENTS.md`](test/AGENTS.md). |
| [`scripts/fetch-jsontestsuite.sh`](scripts/fetch-jsontestsuite.sh) | Fetches the authoritative third-party corpus — **nst/JSONTestSuite** (318 cases: 95 must-accept, 188 must-reject, 35 implementation-defined) — at pinned commit `1ef36fa0` into the **gitignored** `test/jsontestsuite/`. The corpus is never committed. Run by the `pretest` npm script and by Go's `TestMain`; graded by `ts/test/jsontestsuite.test.js` and `go/jsontestsuite_test.go`. |
...
npm test               # pretest fetches the corpus, then tsc --build src + node --test
```
...
## Conformance — the honest baseline

**Both runtimes are currently RED, on purpose.** The suite was made honest
before the parser was touched, so the numbers below are what the parser
actually does. Do not make them look better by weakening a test.

Measured against nst/JSONTestSuite @ `1ef36fa0` (95 `y_` / 188 `n_` / 35
`i_`):

| | must-accept (`y_`) | must-reject (`n_`) | impl-defined (`i_`), vs platform |
|---|---|---|---|
| TypeScript | 95/95 value-correct | **182/188** | 35/35 accept-reject, 35/35 value |
| Go | 95/95 value-correct | **186/188** | 35/35 accept-reject, **25/35** value |

Two defects, and they are *different* in the two runtimes:

1. **TypeScript accepts trailing content** after a complete top-level value
   and silently discards it — `[1]]` → `[1]`, `{}}` → `{}`, `1,` → `1`,
   `"a" "b"` → `"a"`. That is data loss, not laxness. It is base-engine
   leniency (`val.close` has a `{ b: 1, ... }` "there's more JSON"
   alternate, and `rule.finish` is `false`) that `JSON_OPTIONS` does not
   close off. Go rejects all of these.
2. **Go accepts a non-string object key** and coerces it to the empty
   string — `{1:1}` → `{"":1}`, `{null:null,null:null}` → `{"":null}`.
   TypeScript rejects both. Go also keeps invalid UTF-8 bytes verbatim in
   string values where `encoding/json` substitutes U+FFFD (the 10 `i_`
   value mismatches).

Both defects are pinned as `input → ERROR:unexpected` rows in
`test/spec/rfc8259-gaps.tsv`, so they stay visible even without the corpus.

Note the five `i_number_*_huge_exp` / `*_overflow` cases where TS accepts
and Go rejects: that is not a defect on either side — `JSON.parse` accepts
them (as `Infinity`/`0`) and `encoding/json` rejects them, and each runtime
correctly tracks its own platform, which is the declared bar.
