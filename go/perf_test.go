...
// convenience Parse() rebuilds the (expensive) JSON grammar on every call
// instead of reusing a cached instance. Building the engine + grammar
// dominates a parse, so a rebuild-per-call Parse() is many times slower than
// reusing one Make() instance.
//
